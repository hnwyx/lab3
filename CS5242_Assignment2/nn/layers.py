...
class VanillaRNN(Layer):
    ...
    def forward(self, input):
        ...
        for t in range(input.shape[1]):
            ...
            output.append(out)  #outshape: (batch,units)
            h = out
...
class BiRNN(Layer):
    ...
    def backward(self, out_grad, input):
        ...
        ##############################To Do ##############################
        in_grad = None
        units = int(out_grad.shape[2]/2)
        mask = ~np.any(np.isnan(input), axis=2)
        forward_input = input
        backward_input = self._reverse_temporal_data(input, mask)
        forward_grad = self.forward_rnn.backward(out_grad[:,:,:units],forward_input)
        backward_grad = self.backward_rnn.backward(self._reverse_temporal_data(out_grad[:,:,units:2*units],mask),backward_input)
